...
router.get(routes.map((e) => `${e}/:name`), validSchema(apiAppsV1OpenApiV3), general.findOne(Secret), general.format(Secret), general.raw(Secret));

router.get(['/api/v1/secrets', ...routes], validSchema(apiV1OpenApiV3), general.find(Secret), general.format(Secret), general.list(Secret));
